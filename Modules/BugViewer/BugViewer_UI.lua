-- BugViewer: UI for error viewing and management
local BugViewer = _G["BugViewer"]

-- Main frame
local frame = CreateFrame("Frame", "BugViewerFrame", UIParent, "BasicFrameTemplateWithInset")
...
frame.title:SetText("BugViewer - Lua Error Log")
...
local function RefreshErrorList()
    ...
    local y = 0
    for i, err in ipairs(BugViewer.errors) do
...
-- Error detail popup
function BugViewer_ShowErrorDetail(idx)
    local err = BugViewer.errors[idx]
...
copyAllBtn:SetScript("OnClick", function()
    local all = {}
    for _, err in ipairs(BugViewer.errors) do
...
clearBtn:SetScript("OnClick", function()
    wipe(BugViewer.errors)
    RefreshErrorList()
end)

-- Show UI command
SLASH_BUGVIEWER1 = "/bugviewer"
SLASH_BUGVIEWER2 = "/errors"
SlashCmdList["BUGVIEWER"] = function()
    RefreshErrorList()
    frame:Show()
end

-- Minimap button (simple)
local mini = CreateFrame("Button", "BugViewerMiniBtn", Minimap)
mini:SetSize(32, 32)
mini:SetNormalTexture("Interface/ICONS/INV_Misc_Bug_01")
mini:SetPoint("TOPLEFT", Minimap, "BOTTOMLEFT", 0, 0)
...
mini:SetScript("OnEnter", function(self)
    ...
    GameTooltip:SetText("BugViewer: Show Lua Errors", 1, 1, 1)
    GameTooltip:Show()
end)
